...
package com.example.skindetection.localize;
...
//import com.example.skindetection.data.CovidRecord;
import com.example.skindetection.localize.customview.OverlayView;
import com.example.skindetection.localize.customview.OverlayView.DrawCallback;
import com.example.skindetection.localize.env.BorderedText;
import com.example.skindetection.localize.env.ImageUtils;
import com.example.skindetection.localize.env.Logger;
import com.example.skindetection.localize.tflite.Classifier;
import com.example.skindetection.localize.tflite.TFLiteObjectDetectionAPIModel;
import com.example.skindetection.localize.tflite.TFLiteObjectDetectionEfficientDet;
import com.example.skindetection.localize.tflite.TFLiteObjectDetectionEfficientDetJustification;
import com.example.skindetection.localize.tracking.MultiBoxTracker;
//import com.example.skindetection.storage.FirebaseStorageUtil;
...
public class DetectorActivity extends CameraActivity implements OnImageAvailableListener {
  ...
  private static final String TF_OD_API_MODEL_FILE = "detect-skin.tflite";   //name of input file for MODEL must be tflite format
  ...
  private static final String TF_OD_API_LABELS_FILE = "file:///android_asset/labelmap-skin.txt";
  ...
  private static final String TF_JUSTIFICATION_MODEL_FILE = "skinDetectionJustification.tflite";   //name of input file for MODEL must be tflite format
  //private static final String TF_OD_API_MODEL_FILE = "maskDetectorSSD.tflite";                                                                      //TIP: if creating subclass for say mask detection make your detector
  //   file called maskdetect.flite and put in assets folder
  private static final String TF_JUSTIFICATION_LABELS_FILE = "file:///android_asset/labels-cnn2.txt";
  ...
  private Classifier detector;  //class variable representing the actual model loaded up
                                // note this is  com.example.skindetection.localize.tflite.Classifier;
  private Classifier justificationDetector;
  ...
  private MultiBoxTracker tracker; // this class assists with tracking bounding boxes - represents results
                                   //note this is instance of com.example.skindetection.localize.tracking.MultiBoxTracker;
  ...
  public void onPreviewSizeChosen(final Size size, final int rotation) {
    ...
    try {
      ...
      justificationDetector =
              TFLiteObjectDetectionEfficientDetJustification.create(
                      getAssets(),
                      TF_JUSTIFICATION_MODEL_FILE,
                      TF_JUSTIFICATION_LABELS_FILE,
                      TF_OD_API_INPUT_SIZE,
                      TF_OD_API_IS_QUANTIZED);
    ...
    runInBackground(
        new Runnable() {
          ...
          public void run() {
            ...
            LOGGER.i("Running detection on image " + croppedBitmap);

            try {
              results = detector.recognizeImage(croppedBitmap);  //performing detection on croppedBitmap
            ...
            for (final Classifier.Recognition result : results) {  //loop variable is result, represents one detection
              ...
              if (location != null && result.getConfidence() >= minimumConfidence) { //ONLY display if the result has a confidence > threshold
                ...
                /*if(CovidRecord.readyStoreRecord(MainActivity.covidRecordLastStoreTimestamp, MainActivity.deltaCovidRecordStoreTimeMS, MainActivity.covidRecordLastStoreLocation, MainActivity.currentLocation, MainActivity.deltaCovidRecordStoreLocationM)) {
                  Date d = new Date();
                  ...
                  CovidRecord myRecord = new CovidRecord(90.0f, result.getConfidence()*100,
                          new GeoPoint(MainActivity.currentLocation.getLatitude(), MainActivity.currentLocation.getLongitude()),
                          Timestamp.now(), imageFileURL, result.getTitle(),boundingBox, angles, 0.0f,
                          MainActivity.userEmailFirebase, MainActivity.userIdFirebase, "covidRecord");


                  FirebaseStorageUtil.storeImageAndCovidRecord(cropCopyBitmap, myRecord, MainActivity.currentLocation, "covidRecord");
                }*/
                //###############################################
            ...
            Classifier.Recognition topRecognition = results.get(0);
            if(topRecognition.getTitle().contains("Malignant") && topRecognition.getConfidence() > minimumConfidence) {
              ...
              List<Classifier.Recognition> justificationResults = null;

              try {
                justificationResults = justificationDetector.recognizeImage(croppedBitmap);  //performing detection on croppedBitmap
              }catch(NullPointerException ignored){

              }
              int saveImageOnceFlag1 = 1;
              String imageFileURL1 = "";

              for (final Classifier.Recognition justificationResult : justificationResults) {  //loop variable is result, represents one detection
                final RectF justificationLocation = justificationResult.getLocation();  //getting as  a rectangle the bounding box of the result detecgiton
                if (justificationLocation != null && justificationResult.getConfidence() >= minimumConfidence) { //ONLY display if the result has a confidence > threshold
                  canvas.drawRect(justificationLocation, paint2);  //draw in the canvas the bounding boxes-->

                  canvas.drawText(justificationResult.getTitle(),justificationLocation.left,justificationLocation.top, paintText); //draw out label aslo
                  //Take our bounding box (location) and for the purpose of DISPALY we need to transform its location
                  // to the units for display from the cropSizexcropSize used as input to model (e.g. 512x512)

                  cropToFrameTransform.mapRect(justificationLocation);  //transforms using Matrix the bounding box to the correct transformed coordinates

                  justificationResult.setLocation(justificationLocation); // reset the newly transformed rectangle (location) representing bounding box inside the result
                  mappedRecognitions.add(justificationResult);  //add the result to a linked list used in the OverlayView for drawing

                  //==============================================================
                  //COVID: code to store image to CloudStore (if any results have result.getConfidence() > minimumConfidence
                  //  ONLY store one time regardless of number of recognition results.
                  if(saveImageOnceFlag1 == 1){

                    //set flag so know have already stored this image
                    saveImageOnceFlag1 = 0;
                ...
                /*if(CovidRecord.readyStoreRecord(MainActivity.covidRecordLastStoreTimestamp, MainActivity.deltaCovidRecordStoreTimeMS, MainActivity.covidRecordLastStoreLocation, MainActivity.currentLocation, MainActivity.deltaCovidRecordStoreLocationM)) {
                  ...
                  justificationResult.setLocation(justificationLocation); // reset the newly transformed rectangle (location) representing bounding box inside the result
                  mappedRecognitions.add(justificationResult);  //add the result to a linked list
